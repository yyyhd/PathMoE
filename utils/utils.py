...
def collate_MIL1(batch):
	img1 = torch.cat([item[0] for item in batch], dim = 0)
	...
	return [img1, img2, img3, img4, img5, label, slide_id]

def collate_MIL2(batch):
	img1 = torch.cat([item[0] for item in batch], dim = 0)
	img2 = torch.cat([item[1] for item in batch], dim=0)
	img3 = torch.cat([item[2] for item in batch], dim = 0)
	img4 = torch.cat([item[3] for item in batch], dim=0)
...
def collate_MIL3(batch):
	img1 = torch.cat([item[0] for item in batch], dim = 0)
	img2 = torch.cat([item[1] for item in batch], dim=0)
	coord1 = torch.LongTensor([item[2] for item in batch])
	coord2 = torch.LongTensor([item[3] for item in batch])
...
def get_simple_loader1(dataset, batch_size=1, num_workers=1):
	kwargs = {'num_workers': 4, 'pin_memory': False, 'num_workers': num_workers} if device.type == "cuda" else {}
	loader = DataLoader(dataset, batch_size=batch_size, sampler = sampler.SequentialSampler(dataset), collate_fn = collate_MIL2, **kwargs)
	return loader

def get_simple_loader2(dataset, batch_size=1, num_workers=1):
	kwargs = {'num_workers': 4, 'pin_memory': False, 'num_workers': num_workers} if device.type == "cuda" else {}
	loader = DataLoader(dataset, batch_size=batch_size, sampler = sampler.SequentialSampler(dataset), collate_fn = collate_MIL3, **kwargs)
	return loader
...
def get_split_loader1(split_dataset, training=False, testing=False, weighted=False):
	"""
		return either the validation loader or training loader
	"""
	kwargs = {'num_workers': 4} if device.type == "cuda" else {}
	if not testing:
		if training:
			if weighted:
				...
				loader = DataLoader(split_dataset, batch_size=1, sampler=WeightedRandomSampler(weights, len(weights)),
									collate_fn=collate_MIL1, **kwargs)
			else:
		...
		ids = np.random.choice(np.arange(len(split_dataset), int(len(split_dataset) * 0.1)), replace=False)
		loader = DataLoader(split_dataset, batch_size=1, sampler=SubsetSequentialSampler(ids), collate_fn=collate_MIL1,
							**kwargs)
